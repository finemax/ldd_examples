...
int scull_read_mem_proc_show(struct seq_file *m, void *v)
{
	int i, j, len = 0;
...
#define DEFINE_PROC_SEQ_FILE(__name) \
    static int __name##_proc_open(struct inode *inode, struct file *file) \
    {\
        return single_open(file, __name##_proc_show, NULL); \
    }\
    \
    static const struct file_operations __name##_proc_fops = {\
